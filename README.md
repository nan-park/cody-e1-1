...
아래 미션이 전부 터미널에서 진행되는 이유이기도 합니다.
...
가장 먼저 잡아야 할 개념은 **이미지와 컨테이너의 구분**입니다. 이 문서 전체가 이 구분 위에 서 있습니다.

| | 이미지 (image) | 컨테이너 (container) |
|---|---|---|
| 정체 | **읽기 전용 틀** — 파일들의 스냅샷 | 이미지를 실행한 **프로세스** |
| 비유 | 붕어빵 **틀** / 프로그램 설치 파일 | 붕어빵 / 실행 중인 프로그램 |
...
```
  Dockerfile  ──build──▶   이미지   ──run──▶   컨테이너  ──▶  컨테이너
  (레시피/텍스트)         (읽기 전용 틀)      (실행 중)      (같은 이미지로 여러 개)
   [3절]                    ▲   │                 │
...
**VM(가상 머신)과의 차이**도 여기서 갈립니다. VM은 게스트 OS 전체(커널 포함)를 통째로 띄우지만,
...
일반적인 서버 프로그램은 로그를 파일로 남기지만, **컨테이너에서는 PID 1이 화면에 출력한 내용(표준출력)이 곧 로그**입니다.
`docker logs`는 그 출력을 대신 받아서 보여주는 것뿐입니다. 로그가 "파일"이 아니라 "출력 스트림"이라는 걸,
아래처럼 PID 1의 stdout(`/proc/1/fd/1`)에 직접 글자를 써넣어 확인했습니다.
...
**하나의 이미지 · 서로 다른 설정 · 동시에 실행** — 이것이 환경 변수를 쓰는 이유입니다.

> ⚠️ 다만 **비밀번호·토큰 같은 민감한 값**은 이렇게 넣지 않습니다.
> `-e`로 준 값은 `docker inspect`와 셸 히스토리에 그대로 남기 때문에, 그런 값은 Docker secret이나 별도 설정 저장소를 씁니다.
